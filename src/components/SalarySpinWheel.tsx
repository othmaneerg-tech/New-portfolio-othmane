...
export default function SalarySpinWheel() {
  ...
  return (
    <div className="flex flex-col items-center mt-20 mb-10 w-full px-4 relative z-10">
      {/* Playful Header */}
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-16 w-full p-8 min-h-[600px] relative">
        ...
        <div className="relative z-10 flex flex-col items-center">
          <div className="relative w-80 md:w-[450px]">
            {/* Updated to Avatar11 and made bigger */}
            <Image
            ...
            <motion.div
              ...
              className={`absolute transition-all duration-500 bg-[#111]/90 backdrop-blur-md p-5 rounded-2xl rounded-bl-none border border-[#bf953f]/40 shadow-[0_0_40px_rgba(191,149,63,0.2)] z-20 max-w-[200px] md:max-w-none ${
                showResult 
                  ? "top-[3%] -right-16 md:-right-32" 
                  : "top-[5%] -right-12 md:-right-20"
              }`}
            >
              <h4 className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#bf953f] to-[#fcf6ba] text-xl mb-1">
                Welcome!
              </h4>
        ...
        <div className="relative z-10 flex flex-col items-center mt-12 md:mt-0">
          ...
          {/* 3D Gold Stand Base */}
          <div className="absolute -bottom-16 w-32 h-24 bg-gradient-to-b from-[#b38728] via-[#bf953f] to-[#aa771c]" style={{ clipPath: 'polygon(20% 0%, 80% 0%, 100% 100%, 0% 100%)', boxShadow: 'inset 0px 10px 20px rgba(0,0,0,0.5)' }} />
          <div className="absolute -bottom-16 w-40 h-4 bg-gradient-to-r from-[#aa771c] via-[#fcf6ba] to-[#aa771c] rounded-sm translate-y-24 shadow-2xl" />

          {/* The Outer Gold Frame */}
          <div className="relative w-80 h-80 md:w-[450px] md:h-[450px] rounded-full p-4 bg-gradient-to-br from-[#bf953f] via-[#fcf6ba] to-[#aa771c] shadow-[0_20px_50_rgba(0,0,0,0.5),inset_0_5px_20px_rgba(255,255,255,0.8)]">
            ...
            <div className="absolute inset-0 rounded-full z-20 pointer-events-none">
              {[...Array(16)].map((_, i) => (
                <div
                  key={i}
                  className="absolute w-2.5 h-2.5 md:w-3 md:h-3 rounded-full bg-white shadow-[0_0_15px_2px_#fff,0_0_5px_1px_#fcf6ba]"
                  style={{
                    top: `calc(50% - ${Math.cos((i * 22.5 * Math.PI) / 180) * 48.5}% - 6px)`,
                    left: `calc(50% + ${Math.sin((i * 22.5 * Math.PI) / 180) * 48.5}% - 6px)`
                  }}
                />
              ))}
            </div>
            ...
            {/* 3D Pointer / Flapper */}
            <div className="absolute -top-6 left-1/2 -translate-x-1/2 z-40 drop-shadow-[0_5px_5px_rgba(0,0,0,0.5)] flex flex-col items-center">
              <div className="w-8 h-6 bg-[#111] rounded-t-md border-t-2 border-x-2 border-[#bf953f]" />
              <div className="w-0 h-0 border-l-[16px] border-r-[16px] border-t-[28px] border-l-transparent border-r-transparent border-t-[#111] relative">
                {/* Gold trim on the pointer */}
                <div className="absolute -top-[28px] -left-[12px] w-0 h-0 border-l-[12px] border-r-[12px] border-t-[22px] border-l-transparent border-r-transparent border-t-[#bf953f]" />
              </div>
            </div>
            ...
            >
              {/* Adding textured gradient overlay to segments for 3D depth */}
              ...
              {salaries.map((salary, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="absolute w-full h-full flex justify-center items-start pt-8 md:pt-12 font-bold text-sm md:text-xl tracking-tighter"
                    style={{
            ...
            {/* Luxury Center Pin */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 md:w-20 md:h-20 z-30">
              <div className="w-full h-full rounded-full bg-gradient-to-br from-[#bf953f] via-[#fcf6ba] to-[#aa771c] shadow-2xl border-4 border-[#111] flex items-center justify-center overflow-hidden">
                ...
                >
                  <span className="text-[#111] font-black tracking-widest text-xs md:text-sm uppercase relative z-10">
                    {isSpinning ? "???" : "SPIN"}
                  </span>
                </button>
              </div>
